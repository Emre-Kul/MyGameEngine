...
bool Collision2D::RecRecCol(SHP2Rectangle &rec1,SHP2Rectangle &rec2){
	if(rec1.vecMin.x > rec2.vecMax.x) return false;
	else if(rec1.vecMax.x < rec2.vecMin.x) return false;
	else if(rec1.vecMin.y > rec2.vecMax.y) return false;
	else if(rec1.vecMax.y < rec2.vecMin.y ) return false;
	return true;
}
